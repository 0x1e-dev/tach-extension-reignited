...
class Kavita(
    ...
    override val name = "${KavitaInt.KAVITA_NAME} (${preferences.getString(KavitaConstants.CUSTOM_SOURCE_NAME_PREF, suffix)})"
    ...
    private val logTag = """Kavita_${"[$suffix]_" +
        preferences
            .getString(
                KavitaConstants.CUSTOM_SOURCE_NAME_PREF,
                "[$suffix]",
            )!!
            .replace(' ', '_')
    }"""
    ...
    private val cacheTtl = 30 * 60 * 1000L // 30 minutes in milliseconds
    ...
    private inline fun <reified T> Response.parseAs(): T =
        use {
            if (!it.isSuccessful) {
                ...
                Log.e(logTag, "HTTP error ${it.code}: $errorMessage - ${it.request.url}")
                val versionCheck =
                    try {
                        intl["check_version"]
                    } catch (e: Exception) {
            ...
            if (body.isEmpty()) {
                Log.e(logTag, "Empty body for: ${it.request.url}")
        ...
        return try {
            when {
                ...
                manga.url.contains("/series/") || manga.url.contains("/Series/") -> {
                    ...
                    if (seriesId == null) {
                        Log.w(logTag, "Couldn't extract series ID from URL: ${manga.url}")
                        return manga.url
                    }
            ...
            Log.e(logTag, "Error in getMangaUrl", e)
    ...
    override fun getChapterUrl(chapter: SChapter): String {
        ...
        return chapter.url.substringBefore("_").substringBefore("?") // strips fileCount/split marker if appended
    }
     ...
     */
    private suspend fun fetch(request: Request): MangasPage =
        withContext(Dispatchers.IO) {
            try {
                ...
                Log.e(logTag, "Error fetching manga", e)
                throw e
            }
        }
    ...
    override fun popularMangaParse(response: Response): MangasPage {
        try {
            ...
            Log.d(logTag, "Parsing ${result.size} series for popular feed")

            val allManga =
                result.map<SeriesDto, SManga> { item ->
                    helper.createSeriesDto(item, apiUrl, apiKey)
                }

            Log.d(logTag, "Created ${allManga.size} titles")
            ...
            Log.d(logTag, "After filtering: ${filteredManga.size} titles remain")

            if (filteredManga.isEmpty()) {
                Log.w(logTag, "All titles were filtered out. This might indicate a library name mismatch.")
                Log.d(logTag, "Available library names in series: ${result.map { it.libraryName }.distinct()}")
                Log.d(logTag, "Allowed libraries from preferences: ${preferences.allowedLibrariesFeed}")
            }

            return MangasPage(filteredManga, helper.hasNextPage(response))
        } catch (e: Exception) {
            Log.e(logTag, "Unhandled exception", e)
    ...
    override fun searchMangaRequest(
        ...
        if (genresListMeta.isEmpty() || tagsListMeta.isEmpty()) {
            Log.w(logTag, "Metadata not loaded, retrying filters")
        ...
        Log.d(logTag, "Building search request for query: '$query' with filters: $filters")
            ...
            client.newCall(request).execute().use {
                if (it.code == 200) {
                    ...
                    return POST(
                        url,
                        headersBuilder().build(),
                        json.encodeToJsonElement(decodedFilter).toString().toRequestBody(jsonMediaType),
                    )
                } else {
                    Log.e(logTag, "Failed to decode SmartFilter: ${it.code}\n" + it.message)
            ...
            when {
                queryLower.startsWith("artist:") -> {
                    ...
                    if (artistQuery.isNotBlank()) {
                        Log.d(logTag, "Searching for artist (text contains): '$artistQuery'")
                ...
                queryLower.startsWith("people:") -> {
                    ...
                    if (peopleQuery.isNotBlank()) {
                        Log.d(logTag, "Searching for people (text contains): '$peopleQuery'")
        ...
        filters.forEach { filter ->
            when (filter) {
                    ...
                    when (filter) {
                        ...
                        is GenreFilterGroup -> {
                            filter.state.forEach { genreFilter ->
                                ...
                                Log.d(logTag, "Checking genre: ${genreFilter.name}, state=${genreFilter.state}, match=${genre?.id}")
                                genre?.let {
                        ...
                        is PublisherPeopleFilterGroup -> {
                            ...
                            Log.d(logTag, "Unhandled filter group type: ${filter.javaClass.simpleName}")
                        }
                    }
                }
                ...
                is Filter.CheckBox -> {
                    ...
                    Log.d(logTag, "Standalone checkbox filter: ${filter.name}")
                }

                is Filter.Text -> {
                    // Handle standalone text filters
                    Log.d(logTag, "Standalone text filter: ${filter.name}")
                }

                is Filter.TriState -> {
                    // Handle standalone tri-state filters
                    Log.d(logTag, "Standalone tri-state filter: ${filter.name}")
                }
                ...
                is Filter.Separator -> {
                    ...
                    Log.d(logTag, "Unhandled filter type: ${filter.javaClass.simpleName}")
                }
            }
        }
        ...
        Log.d(logTag, "Search payload: $payload")
        Log.d(logTag, "Filter statements: ${filterV2.statements}")
        ...
        runAsObservable {
            ...
            if (manga.url.contains("source=readinglist")) {
                ...
                if (readingList != null) {
                    ...
                    val starting =
                        readingList.startingYear.takeIf { it > 0 }?.let {
                            "${readingList.startingYear}-${readingList.startingMonth.toString().padStart(2, '0')}"
                        }
                    val ending =
                        readingList.endingYear.takeIf { it > 0 }?.let {
                    ...
                    val items =
                        try {
                            ...
                            Log.e(logTag, "Error parsing reading list items", e)
                            emptyList()
                        }
                    ...
                    val allGenres =
                        items
                            .flatMap { item ->
                                try {
                                    val seriesMetaRequest =
                                        GET("$apiUrl/series/metadata?seriesId=${item.seriesId}", headersBuilder().build())
                                    client
                                        .newCall(seriesMetaRequest)
                                        .await()
                                        .parseAs<SeriesDetailPlusDto>()
                                        .genres
                                        .map { it.title }
                                } catch (e: Exception) {
                                    Log.e(logTag, "Error fetching series metadata for item ${item.seriesId}", e)
                                    emptyList()
                                }
                            }.distinct()
    ...
    private fun resolveThumbnailUrl(seriesId: Int?): String {
        ...
        return try {
            Log.d(logTag, "Fetching volumes for series $seriesId")
            val volumes =
                client
                    .newCall(
            ...
            Log.d(logTag, "Found ${coverCandidates.size} cover candidates")
            ...
            Log.e(logTag, "Error fetching volumes for cover selection", e)
            staticCover
        }
    }
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        val result =
            try {
                ...
                Log.e(logTag, "Error parsing series details", e)
            ...
            try {
                val responsePlus =
                    try {
                        ...
                        Log.w(logTag, "series-detail-plus endpoint failed, falling back to ratings endpoint", e)
                        null
                    }
                ...
                if (responsePlus == null || !responsePlus.isSuccessful) {
                    ...
                    if (rawResponse.isBlank() || rawResponse.trim().startsWith("<")) {
                        Log.e(logTag, "Invalid response for series-detail-plus: $rawResponse")
                        val score = result.ratings.firstOrNull()?.averageScore ?: 0f
                        if (score > 0) "⭐ Score: ${"%.1f".format(score)}\n" else ""
                    } else {
                        try {
                            ...
                            val score =
                                detailPlusWrapper.series?.averageScore?.takeIf { it > 0f }
                                    ?: detailPlusWrapper.ratings?.firstOrNull()?.averageScore
                                    ?: result.ratings.firstOrNull()?.averageScore
                                    ?: 0f
                            if (score > 0) "⭐ Score: ${"%.1f".format(score)}\n" else ""
                        } catch (e: Exception) {
                            Log.e(logTag, "Error parsing series detail plus, using fallback", e)
                            val score = result.ratings.firstOrNull()?.averageScore ?: 0f
                            if (score > 0) "⭐ Score: ${"%.1f".format(score)}\n" else ""
                        }
                    }
                ...
                Log.e(logTag, "Error fetching ratings, using fallback", e)
                val score = result.ratings.firstOrNull()?.averageScore ?: 0f
                if (score > 0) "⭐ Score: ${"%.1f".format(score)}\n" else ""
            }
...
//            Log.d(logTag, "Publication status received: ${result.publicationStatus}")
        }
    }
    ...
    private fun readingListParse(response: Response): MangasPage =
        try {
            ...
            Log.e(logTag, "Error parsing reading lists", e)
        ...
        val isWebtoon =
            firstItem?.let { item ->
                runCatching<Boolean> {
                    val meta =
                        client
                            .newCall(GET("$apiUrl/series/metadata?seriesId=${item.seriesId}", headersBuilder().build()))
                            .await()
                            .parseAs<SeriesDetailPlusDto>()
                    ...
                    val allTagsForGrouping =
                        if (preferences.groupTags) {
                            emptyList()
                        } else {
                    ...
                    val libraryName =
                        meta.libraryName.takeIf { !it.isNullOrEmpty() }
                            ?: series.find { it.id == item.seriesId }?.libraryName
                            ?: runCatching {
                                val seriesDto =
                                    client
                                        .newCall(GET("$apiUrl/Series/${item.seriesId}", headersBuilder().build()))
                                        .await()
                                        .parseAs<SeriesDto>()
                                seriesDto.libraryName
                            }.getOrNull()

                    Log.d(logTag, "Reading list item ${item.seriesId} library: $libraryName")

                    val isWebtoonDetected =
                        helper.isWebtoonOrLongStrip(
                            genres = genreTitles,
                            tags = tagTitles,
        ...
        return items
            .map { item ->
                SChapter.create().apply {
                    url =
                        when {
                            item.chapterId != null && item.chapterId > 0 -> {
                                "/Chapter/${item.chapterId}?readingListId=$readingListId" +
                                    "&seriesId=${item.seriesId}&chapterId=${item.chapterId}"
                            }
                        ...
                        when {
                            item.volumeId != null && item.volumeNumber != null -> {
                                when {
                                    ...
                                    item.chapterNumber == KavitaConstants.UNNUMBERED_VOLUME_STR -> {
                    ...
                    date_upload =
                        if (preferences.RdDate && !item.releaseDate.isNullOrBlank()) {
    ...
    override fun relatedMangaListRequest(manga: SManga): Request =
        if (manga.url.contains("/ReadingList/") || manga.url.contains("readingListId=")) {
            val readingListId = manga.url.substringAfter("readingListId=").substringBefore("&")
            ...
            Log.d(logTag, "Fetching related manga from: $url")
            ...
            if (requestUrl.contains("/ReadingList/items")) {
                ...
                items.distinctBy { it.seriesId }.mapNotNull { item ->
                    try {
                        ...
                        Log.e(logTag, "Error fetching series for related reading list", e)
                        null
                    }
                }
            } else {
                ...
                if (rawResponse.isEmpty() || rawResponse.startsWith("<")) {
                    Log.e(logTag, "Invalid response for related manga: $rawResponse")
                    return emptyList()
                }
                ...
                Log.d(logTag, "Processing suggestions for current manga library: $currentMangaLibrary")
                Log.d(logTag, "Total related items before filtering: ${filteredRelated.size}")

                val suggestions =
                    filteredRelated.mapNotNull { (item, relation) ->
                        val suggestionLibraryName =
                            item.libraryName
                ...
                Log.d(logTag, "Final suggestions count: ${suggestions.size}")
                Log.d(logTag, "Allowed libraries for search: ${preferences.allowedLibrariesSearch}")
                suggestions
            }
        } catch (e: Exception) {
            Log.e(logTag, "Error parsing related manga", e)
            emptyList()
        }
    }
    ...
    private fun getLibraryType(seriesId: Int): LibraryTypeEnum =
        libraryTypeCache[seriesId] ?: run {
            try {
                val seriesDto =
                    client
                        .newCall(GET("$apiUrl/Series/$seriesId", headersBuilder().build()))
                        .execute()
                        .parseAs<SeriesDto>()
                ...
                val type =
                    libraryListMeta.find { it.id == seriesDto.libraryId }?.type?.let { typeInt ->
                        ...
                        val library =
                            client
                                .newCall(GET("$apiUrl/Library/${seriesDto.libraryId}", headersBuilder().build()))
                                .execute()
                                .parseAs<LibraryDto>()
                ...
                Log.e(logTag, "Error getting library type for series $seriesId", e)
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        try {
            ...
            val seriesName =
                seriesId?.let { id: Int ->
                    ...
                    series.find { it.id == id }?.name
                        ?: runCatching {
                            client
                                .newCall(GET("$apiUrl/Series/$id", headersBuilder().build()))
                                .execute()
                                .parseAs<SeriesDto>()
                                .name
                        }.getOrNull()
            ...
            Log.d(logTag, "Processing chapters for series $seriesId with name: '$seriesName'")
            ...
            val isWebtoon =
                seriesId?.let {
                    runCatching<Boolean> {
                        val meta =
                            client
                                .newCall(GET("$apiUrl/series/metadata?seriesId=$it", headersBuilder().build()))
                                .execute()
                                .parseAs<SeriesDetailPlusDto>()
                        val genreTitles = meta.genres.map { it.title }
                        val tagTitles = meta.tags.map { it.title }
                        ...
                        Log.d(logTag, "Checking webtoon detection for series $seriesId:")
                        Log.d(logTag, "  Genres: $genreTitles")
                        Log.d(logTag, "  Tags: $tagTitles")
                        Log.d(logTag, "  Library: $libraryName")
                        Log.d(logTag, "  Formats: $foundFormats")
                        Log.d(logTag, "  Demographic: $foundDemographic")
                        Log.d(logTag, "  AllTags (GroupTags off): $allTagsForGrouping")
                        ...
                        Log.d(logTag, "Webtoon detected for series $seriesId: $isWebtoonDetected")
                        isWebtoonDetected
                    }.getOrElse {
                        Log.e(logTag, "Error checking webtoon status for series $seriesId", it)
                        false
                    }
                } ?: false
            ...
            seriesId?.let { id: Int ->
                ...
                series.find { it.id == id }?.let { sDto: SeriesDto ->
                    ...
                    runCatching {
                        ...
                        Log.e(logTag, "Failed to fetch series $id for context", e)
                    }
                }
            }
            ...
            volumes.forEach { volume ->
                ...
                val isSingleFileVolume =
                    volume.chapters.size == 1 &&
            ...
            Log.e(logTag, "Unhandled exception parsing chapters", e)
            throw IOException(intl["version_exceptions_chapters_parse"])
        }
    }
    ...
    override fun pageListRequest(chapter: SChapter): Request {
        ...
        if (chapter.url.contains("readingListId=")) {
            ...
            val chapterItem =
                items.find { it.seriesId == seriesId && it.chapterId != null }
        ...
        runAsObservable {
            ...
            if (chapter.url.contains("readingListId=")) {
                try {
                    ...
                    val chapterDetails =
                        when {
                            ...
                            listItem.volumeId != null -> {
                                ...
                                throw IOException(intl["error_invalid_reading_list_item"])
                            }
                        }
                    ...
                    Log.e(logTag, "Error processing reading list item", e)
                    throw e
                }
            }
            ...
            try {
                ...
                if (chapter.url.contains("/Volume/") || chapter.url.startsWith("volume_")) {
                    val volumeId =
                        when {
                            ...
                            chapter.url.contains("/Volume/") -> {
                                ...
                                throw IOException("Invalid volume URL format")
                            }
                        }
                    ...
                    val volume = client.newCall(volumeRequest).await().parseAs<VolumeDto>()
                    val matchingChapter =
                        volume.chapters.firstOrNull() // or match a chapterId if available
                            ?: throw IOException(intl["error_no_chapters_found"])

                    val initialPages =
                        (0 until matchingChapter.pages).map { i ->
                            Page(
                                index = i,
                    ...
                    val chapterId =
                        when {
                            chapter.url.startsWith("chapter_") -> {
                                chapter.url
                            ...
                            chapter.url.contains("/Chapter/") -> {
                                chapter.url.substringAfter("/Chapter/").substringBefore("?")
                            }

                            else -> {
                                throw IOException("Invalid chapter URL format")
                            }
                        }
                ...
                Log.e(logTag, "Error fetching chapter details, using fallback", e)
                ...
                return@runAsObservable (0 until fallbackPageCount).map { i ->
                    Page(
                        index = i,
                        imageUrl =
                            "$apiUrl/Reader/image?chapterId=${chapter.url.substringBefore("_")}" +
                                "&page=$i&extractPdf=true&apiKey=$apiKey",
                    )
                }
            }
        }
    ...
    override fun searchMangaParse(response: Response): MangasPage =
        try {
            ...
            Log.e(logTag, "Error parsing search results", e)
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        val customSourceNamePref =
            EditTextPreference(screen.context).apply {
                key = KavitaConstants.CUSTOM_SOURCE_NAME_PREF
                ...
                setOnPreferenceChangeListener { _, newValue ->
                    val res =
                        preferences
                            .edit()
                            .putString(KavitaConstants.CUSTOM_SOURCE_NAME_PREF, newValue.toString())
                            .commit()
                    Toast
                        .makeText(
                            screen.context,
                    ...
                    Log.v(logTag, "[Preferences] Successfully modified custom source name: $newValue")
                    res
                }
            }
        ...
        val allowedLibrariesFeedPref =
            MultiSelectListPreference(screen.context).apply {
                key = KavitaConstants.ALLOWED_LIBRARIES_FEED_PREF
                ...
                setOnPreferenceChangeListener { _, newValue ->
                    ...
                    preferences
                        .edit()
                        .putStringSet(KavitaConstants.ALLOWED_LIBRARIES_FEED_PREF, checkValue)
                        .commit()
                }
            }

        val allowedLibrariesSearchPref =
            MultiSelectListPreference(screen.context).apply {
                key = KavitaConstants.ALLOWED_LIBRARIES_SEARCH_PREF
                ...
                setOnPreferenceChangeListener { _, newValue ->
                    ...
                    preferences
                        .edit()
                        .putStringSet(KavitaConstants.ALLOWED_LIBRARIES_SEARCH_PREF, checkValue)
                        .commit()
                }
            }
    ...
    private fun PreferenceScreen.editTextPreference(
        ...
        EditTextPreference(context).apply {
            ...
            setOnPreferenceChangeListener { _, newValue ->
                try {
                    // We don't allow to have multiple sources with same ip or domain
                    ...
                    setupLogin(newValue)
                    Log.v(logTag, "[Preferences] Successfully modified OPDS URL")
                    res
                } catch (e: OpdsurlExistsInPref) {
                    Log.e(logTag, "Url exists in a different sourcce")
                    false
                } catch (e: Exception) {
                    Log.e(logTag, "Unrecognised error", e)
                    false
                }
            }
        }
    ...
    private val SharedPreferences.chapterTitleFormat: String
        get() =
            getString(CHAPTER_TITLE_FORMAT_PREF, CHAPTER_TITLE_FORMAT_DEFAULT)
                .orEmpty()
    ...
    private val SharedPreferences.allowedLibrariesFeed: Set<String>
        get() = getStringSet(KavitaConstants.ALLOWED_LIBRARIES_FEED_PREF, emptySet()) ?: emptySet()

    private val SharedPreferences.allowedLibrariesSearch: Set<String>
        get() = getStringSet(KavitaConstants.ALLOWED_LIBRARIES_SEARCH_PREF, emptySet()) ?: emptySet()
    ...
    private fun isLibraryAllowedForFeed(manga: SManga): Boolean {
        ...
        // Debug logging
        Log.d(logTag, "Feed filtering check:")
        Log.d(logTag, "  Allowed libraries: $allowedLibraries")
        Log.d(logTag, "  Manga URL: ${manga.url}")
        Log.d(logTag, "  Extracted seriesId: $seriesId")
        Log.d(logTag, "  Found series: ${series?.name}")
        Log.d(logTag, "  Series libraryName: ${series?.libraryName}")

        if (series == null) {
            val allow = allowedLibraries.isEmpty()
            val reason = if (allow) "no feed restriction set - allowing" else "blocking (cannot resolve library to apply feed filter)"
            Log.w(logTag, "  Series not found in cache, $reason")
            return allow
        }

        val libraryName = series.libraryName
        val isAllowed = allowedLibraries.contains(libraryName)
        Log.d(logTag, "  Library '$libraryName' allowed: $isAllowed")
    ...
    private fun isLibraryAllowedForSearch(manga: SManga): Boolean {
        ...
        val series = series.find { it.id == seriesId }
        if (series == null) {
            Log.d(logTag, "Search filter: series not in cache for url=${manga.url}, blocking because allowedLibrariesSearch is set.")
            return false
        }

        val libraryName = series.libraryName
        val allowed = libraryName != null && allowedLibraries.contains(libraryName)
        Log.d(logTag, "Search filter: library='$libraryName' allowed=$allowed")
        return allowed
    }
    ...
    private fun isLibraryAllowedForSuggestions(
        ...
        if (libraryName == null) {
            Log.d(logTag, "Suggestion '${manga.title}' BLOCKED: no library information available")
            return false
        }
        ...
        if (currentMangaLibrary != null && libraryName == currentMangaLibrary) {
            Log.d(logTag, "Suggestion '${manga.title}' ALLOWED: same library as current manga ($libraryName)")
            return true
        }
        ...
        if (allowedLibrariesFeed.isNotEmpty()) {
            ...
            val verdict = if (isAllowedFeed) "ALLOWED" else "BLOCKED"
            Log.d(logTag, "Suggestion '${manga.title}' from '$libraryName' $verdict by feed allowance")
        ...
        if (allowedLibrariesSearch.isNotEmpty()) {
            ...
            val verdict = if (isAllowedSearch) "ALLOWED" else "BLOCKED"
            Log.d(logTag, "Suggestion '${manga.title}' from '$libraryName' $verdict by search allowance")
            return isAllowedSearch
        }

        // No restrictions configured
        Log.d(logTag, "Suggestion '${manga.title}' ALLOWED: no library restrictions configured")
        return true
    }
    ...
    private fun resetAllPreferences() {
        try {
            ...
            preferences
                .edit()
                .putString(CHAPTER_TITLE_FORMAT_PREF, currentChapterTitleFormat)
                .putString(SCANLATOR_FORMAT_PREF, currentScanlatorFormat)
                .commit()
...
//            cachedReadingLists = emptyList()
            ...
            Log.d(logTag, "Successfully reset all OPDS URLs and preferences")
        } catch (e: Exception) {
            Log.e(logTag, "Error resetting preferences", e)
            ...
            if (prefApiUrl.isNotEmpty()) {
                if (prefApiUrl == getCleanedApiUrl(url)) {
                    if (sourceId.toString() != suffix) {
                        return preferences.getString(KavitaConstants.CUSTOM_SOURCE_NAME_PREF, sourceId.toString())!!
                    }
                }
            }
        }
    ...
    private fun setupLogin(addressFromPreference: String = "") {
        Log.v(logTag, "[Setup Login] Starting setup")
        ...
        if (baseUrlSetup.toHttpUrlOrNull() == null) {
            Log.e(logTag, "Invalid URL $baseUrlSetup")
        ...
        Log.v(logTag, "[Setup Login] Setup successful")
    }

    private fun doLogin() {
        Log.d(logTag, "Attempting login with address: ${address.takeIf { it.isNotEmpty() } ?: "EMPTY"}")
        if (address.isEmpty()) {
            Log.e(logTag, "OPDS URL is empty or null")
        ...
        Log.v(logTag, "[Login] Starting login")
        val request =
            POST(
        ...
        noRedirectClient.newCall(request).execute().use {
            ...
            if (it.code == 200) {
                try {
                    ...
                    Log.e(logTag, "Possible outdated kavita", e)
                ...
                Log.e(logTag, "[LOGIN] login request was redirected -> Code: ${it.code}. Location: $location")
                throw LoginErrorException("${intl["login_errors_redirected"]} $location")
            } else {
                if (it.code == 500) {
                    Log.e(
                        logTag,
                        "[LOGIN] login failed. There was some error -> Code: ${it.code}." +
                            "Response message: ${it.message} Response body: $peekbody.",
                    )
                    throw LoginErrorException(intl["login_errors_failed_login"])
                } else {
                    Log.e(
                        logTag,
                        "[LOGIN] login failed. Authentication was not successful -> Code: ${it.code}." +
                            "Response message: ${it.message} Response body: $peekbody.",
                    )
                    throw LoginErrorException(intl["login_errors_failed_login"])
                }
        ...
        Log.v(logTag, "[Login] Login successful")
    }
    ...
    init {
        if (apiUrl.isNotBlank()) {
            scope.launch {
                try {
                    Log.d(logTag, "Starting Kavita extension initialization")

                    try {
                        doLogin()
                    } catch (e: Exception) {
                        Log.e(logTag, "Login failed during initialization", e)
                        ...
                        listOf(
                            async {
                                try {
                                    ...
                                    Log.d(logTag, "Loaded ${genresListMeta.size} genres")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load genres", e)
                                }
                            },
                            async {
                                try {
                                    tagsListMeta = loadMetadata("$apiUrl/Metadata/tags")
                                    Log.d(logTag, "Loaded ${tagsListMeta.size} tags")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load tags", e)
                                }
                            },
                            async {
                                try {
                                    ageRatingsListMeta = loadMetadata("$apiUrl/Metadata/age-ratings")
                                    Log.d(logTag, "Loaded ${ageRatingsListMeta.size} age ratings")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load age ratings", e)
                                }
                            },
                            async {
                                try {
                                    collectionsListMeta = loadMetadata("$apiUrl/Collection")
                                    Log.d(logTag, "Loaded ${collectionsListMeta.size} collections")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load collections", e)
                                }
                            },
                            async {
                                try {
                                    languagesListMeta = loadMetadata("$apiUrl/Metadata/languages")
                                    Log.d(logTag, "Loaded ${languagesListMeta.size} languages")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load languages", e)
                                }
                            },
                            async {
                                try {
                                    libraryListMeta = loadMetadata("$apiUrl/Library/libraries")
                                    Log.d(logTag, "Loaded ${libraryListMeta.size} libraries")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load libraries", e)
                                }
                            },
                            async {
                                try {
                                    peopleListMeta = loadMetadata("$apiUrl/Metadata/people")
                                    Log.d(logTag, "Loaded ${peopleListMeta.size} people")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load people", e)
                                }
                            },
                            async {
                                try {
                                    pubStatusListMeta = loadMetadata("$apiUrl/Metadata/publication-status")
                                    Log.d(logTag, "Loaded ${pubStatusListMeta.size} publication statuses")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load publication statuses", e)
                                }
                            },
                            async {
                                try {
                                    smartFilters = loadMetadata("$apiUrl/filter")
                                    Log.d(logTag, "Loaded ${smartFilters.size} smart filters")
                                } catch (e: Exception) {
                                    Log.e(logTag, "Failed to load smart filters", e)
                                }
                            },
                        )
                    ...
                    try {
                        val serverInfoDto =
                            client
                                .newCall(GET("$apiUrl/Server/server-info-slim", headersBuilder().build()))
                                .execute()
                                .parseAs<ServerInfoDto>()
                        Log.d(logTag, "Kavita version: ${serverInfoDto.kavitaVersion}")

                        // Mark as successfully initialized
                        isInitialized = true
                        Log.d(logTag, "Kavita extension initialization completed successfully")
                    } catch (e: Exception) {
                        Log.e(logTag, "Failed to get server info", e)
                        // Don't fail initialization for server info
                        isInitialized = true
                    }
                } catch (e: Exception) {
                    Log.e(logTag, "Critical initialization error", e)
                    initializationError = e
                }
            }
        } else {
            Log.w(logTag, "API URL is blank, skipping initialization")
        }
    }
    ...
    fun cleanup() {
        ...
        Log.d(logTag, "Cleanup completed - cache cleared")
    }
    ...
    private inline fun <reified T> loadMetadata(url: String): T {
        ...
        if (cachedEntry != null && !cachedEntry.isExpired(cacheTtl)) {
            Log.d(logTag, "Using cached metadata for ${T::class.simpleName}")
            return cachedEntry.data
        }

        Log.d(logTag, "Fetching fresh metadata for ${T::class.simpleName} from $url")
        val data =
            client
                .newCall(GET(url, headersBuilder().build()))
                .execute()
                .parseAs<T>()
    ...
    private fun getCacheStats(): String {
        ...
        val expiredEntries = metadataCache.values.count { it.isExpired(cacheTtl) }
